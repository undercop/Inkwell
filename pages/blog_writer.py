...
from core.blog_writer.graph import blog_graph as app, BLOG_OUTPUT_DIR, BASE_DIR
from core.ui.theme import inject_base_styles, BLOG, BORDER
...
def list_past_blogs() -> List[Path]:
    """Returns .md files in data/blogs/, newest first."""
    files = [p for p in BLOG_OUTPUT_DIR.glob("*.md") if p.is_file()]
...
st.markdown(
    ...
    <div class="blog-hero">
        ...
        <p>Give it a topic — a research-aware LangGraph pipeline plans, drafts, and assembles the post section by section.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    topic = st.text_area("Topic", height=120, key="blog_topic")
    as_of = st.date_input("As-of date", value=date.today(), key="blog_as_of")
    run_btn = st.button("🚀 Generate Blog", type="primary", key="blog_run_btn")
    ...
    if not past_files:
        st.caption("No saved blogs found yet (data/blogs/*.md).")
        ...
        if st.button("📂 Load selected blog", key="blog_load_btn"):
            if selected_md_file:
...
if out:
    ...
    with tab_logs:
        ...
        st.text_area("Event log", value="\n\n".join(st.session_state["logs"][-80:]), height=520, key="blog_log_area")
else:
